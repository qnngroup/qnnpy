...
trigger_v, count_rate = counter.scan_trigger_voltage([-0.1,0.1], counting_time=0.2, num_pts=100)
data_dict = {'trigger_v':trigger_v, 'count_rate':count_rate, 'Ib': Ib}
file_path, file_name  = save_data_dict(data_dict, test_type = 'trigger_level', test_name = test_name,
                        filedir = filedirectry, zip_file=True)
plt.semilogy(trigger_v*1e3, count_rate,'o')
plt.xlabel('trigger level (mV)'); plt.ylabel('count rate (cps)')
...
file_path, file_name  = save_data_dict(data_dict, test_type = 'DCR', test_name = test_name+comments,
                        filedir = filedirectry, zip_file=True)
...
file_path, file_name  = save_data_dict(data_dict, test_type = 'BCR', test_name = test_name,
                        filedir = filedirectry, zip_file=True)
...
data_dict = {'OD':OD, 'laser_power':laser_power, 'LCR':LCR, 'currents':currents, 'trigger_level': trigger_level}
file_path, file_name  = save_data_dict(data_dict, test_type = 'LCR', test_name = test_name+comments,
                        filedir = filedirectry, zip_file=True)
