...
class TcCryo:
    ...
    from qnnpy.functions.tccryo import TcCryo

    config = r'S:\SC\Personal\<...>'
    tc = TcCryo(config)

    # check resistances, then perform a cooldown (assuming fridge is currently warm)
    tc.check_resistances()
    tc.measure_cooldown_warmup(animate=True)
    """
    ...
    def __init__(self, configuration_file):
        ...
        self.rcheck_current = float(self.properties["Resistance check"]["current"])
        self.rcheck_n_repeat = float(self.properties["Resistance check"]["repeat"])
        ...
        self.cool_warm_T_base = float(
            self.properties["Cooldown-Warmup measurement"]["T_base"]
        )
        self.cool_warm_T_max = float(
            self.properties["Cooldown-Warmup measurement"]["T_max"]
        )
    ...
    def _mosaic(self):
        ...
        for row in range(3):
            for col in range(2):
                pos = sample_positions[row][col]
                if pos in self.samples:
                    mosaic[row][col] = self.samples[pos]
        return mosaic

    def _set_plot_labels(self, ax):
        for _, sample in self.samples.items():
            ax[sample].set_title(sample)
            ax[sample].set_xlabel("T [K]")
            ax[sample].set_ylabel("R [Ω]")

    def _set_xlim(self, ax, xlim):
        for _, sample in self.samples.items():
            ax[sample].set_xlim(xlim)

    def _set_ylim(self, ax, ylim):
        for _, sample in self.samples.items():
            ax[sample].set_ylim(ylim)
    ...
    def setup_cooldown_warmup_plot(self, **fig_kw):
        ...
        self.fig_cd_wu, self.ax_cd_wu = plt.subplot_mosaic(mosaic, **fig_kw)
    ...
    def update_tc_plot(self, temperatures, voltages, current, color):
        sample_positions = self.get_sample_positions()
        ...
        for row in range(3):
            for col in range(2):
                ...
                if pos not in self.samples:
                    continue
                sample_name = self.samples[pos]
                sample_idx = sample_indices[pos]
                self.ax_tc[sample_name].plot(
                    temperatures[sample_idx, :],
                    voltages[sample_idx, :] / current,
                    ".",
                    color=color,
                )
    ...
    def update_cooldown_warmup_plot(
        self, tstart, timestamps, temperatures, voltages, current
    ):
        sample_positions = self.get_sample_positions()
        sample_indices = self.get_sample_indices()
        for row in range(3):
            for col in range(2):
                ...
                sample_name = self.samples[pos]
                sample_idx = sample_indices[pos]
                self.ax_cd_wu[sample_name].plot(
                    temperatures[sample_idx, :],
                    voltages[sample_idx, :] / current,
                    ".",
                    color="b",
                ...
                self.ax_cd_wu["temp"].plot(
                    (timestamps[sample_idx, :] - tstart) / 60,
                    temperatures[sample_idx, :],
                    ".",
                    color="b",
                )
    ...
    def pretty_print(self, temperatures, voltages, current):
        sample_indices = self.get_sample_indices()
        text = [["", ""], ["", ""], ["", ""]]
    ...
    def save_data(self, tstart, measurement_type, num_rows, data_dict, header):
        # save data
        sample_indices = self.get_sample_indices()
        dtime = datetime.datetime.fromtimestamp(tstart, datetime.UTC)
    ...
    # measurements
    #################################
    def check_resistances(self, print_vrange=False, save_data=True):
        """
        Checks resistance for multiple positions as specified by config file
        """
        sample_indices = self.get_sample_indices()
        n_samples = len(self.samples)
        temperatures = np.zeros((n_samples, self.rcheck_n_repeat))
        ...
        time.sleep(0.1)
        self.isrc.set_current(self.rcheck_current)
        tstart = time.time()
        for pos, sample_idx in sample_indices.items():
            self.select_mux(pos)
            time.sleep(0.5)
            for i in range(self.rcheck_n_repeat):
                timestamps[sample_idx, i] = time.time()
                compliance[sample_idx, i] = self.isrc.in_compliance()
                T = self.temp.read_temp(channel=self.temp_channel)
                v, vr = self.read_voltage()
                temperatures[sample_idx, i] = T
                voltages[sample_idx, i] = v
                vranges[sample_idx, i] = vr
                time.sleep(0.01)
            if self.isrc.in_compliance():
                print("WARNING: COMPLIANCE")
            print_str = (
                f"{self.samples[pos]}: {eng_string(np.mean(voltages[sample_idx,:]) / self.rcheck_current)}Ω",
                "+/-",
                f"{eng_string(np.std(voltages[sample_idx,:]) / self.rcheck_current)}Ω",
            )
            if print_vrange:
                print_str += f" ranges = {vranges[sample_idx,:]}"
            print(print_str)
    ...
    def measure_tc_high_temp(
        self,
        animate=False,
        save_data=True,
    ):
        """Set setpoint to high temperature, measure as temp increases, then stop"""
        t_stop = self.tc_T_max
        sample_indices = self.get_sample_indices()
        n_samples = len(self.samples)
        if animate:
            self.setup_tc_plot(figsize=(6, 9))
        temperatures = [[] for _ in range(n_samples)]
        voltages = [[] for _ in range(n_samples)]
        vranges = [[] for _ in range(n_samples)]
        compliance = [[] for _ in range(n_samples)]
        timestamps = [[] for _ in range(n_samples)]
        ######################
        # setup heater
        ######################
        self.temp.setup_heater(load=25, range="2.5W", source_channel=self.temp_channel)
        time.sleep(0.1)
        self.temp.set_setpoint(0)
        time.sleep(0.1)
        ...
        try:
            while t_max < t_stop:
                ...
                for pos, sample_idx in sample_indices.items():
                    self.select_mux(pos)
                    sleep(animate, 0.5)
                    for _ in range(self.tc_n_repeat):
                        timestamps[sample_idx] += [time.time()]
                        compliance[sample_idx] += [self.isrc.in_compliance()]
                        T = self.temp.read_temp(channel=self.temp_channel)
                        v, vr = self.read_voltage()
                        temperatures[sample_idx] += [T]
                        voltages[sample_idx] += [v]
                        vranges[sample_idx] += [vr]
                        sleep(animate, 0.01)
                self.pretty_print(
                    np.array(temperatures)[:, -self.tc_n_repeat :],
                    np.array(voltages)[:, -self.tc_n_repeat :],
                    self.tc_current,
                )
                print("")
                if animate:
                ...
                t_max = np.max(np.array(temperatures))
            ...
            if save_data:
                ...
                data_dict = dict(
                    ...
                    vranges=np.array(vranges),
                )
                self.save_data(
                    tstart,
                    "tc",
                    {
                        pos: len(timestamps[sample_indices[pos]])
                        for pos in self.samples.keys()
                    },
                    data_dict,
                    header=True,
    ...
    def measure_tc(
        ...
        if t_max > 25:
            raise ValueError(
                "cannot heat above 25 C with coldhead on; use measure_tc_high_temp instead"
            )
        ...
        time.sleep(0.1)
        self.temp.set_setpoint(tlist[0])
        time.sleep(0.1)
        self.temp.start_heater()
        time.sleep(0.1)
        self.temp.set_control_type_pid()
        ...
        try:
            ...
            for ti, t in enumerate(tlist):
                if self.get_power(tprev) != self.get_power(t):
                    self.temp.set_power(self.get_power(t))
                if self.get_pid(tprev) != self.get_pid(t):
                    proportional, integral = self.get_pid(t)
                    self.temp.set_pid(proportional, integral, 0.0)
                self.temp.set_setpoint(t)
    ...
    def measure_cooldown_warmup(
        ...
        if abs(start_temp - self.cool_warm_T_base) < abs(
            start_temp - self.cool_warm_T_max
        ):
            cooldown = False
        ...
        if not cooldown:
            self.temp.setup_heater(
                load=25, range="25W", source_channel=self.temp_channel
            )
            time.sleep(0.1)
            self.temp.set_setpoint(295)
            time.sleep(0.1)
            self.temp.start_heater()
            time.sleep(0.1)
            self.temp.set_control_type_pid()
        ...
        try:
            while True:
                ...
                for save_iter in range(self.cool_warm_save_interval):
                    ...
                    self.pretty_print(
                        temperatures[
                            :, print_start : print_start + self.cool_warm_n_repeat
                        ],
                        voltages[
                            :, print_start : print_start + self.cool_warm_n_repeat
                        ],
