...
class Resonators():
    ...
    def plot(self, close=True):
        ...
        qf.plot(self.f, self.S21_to_dBm(),
                title=self.sample_name+' '+self.device_type+' '+self.device_name+' '+str(self.pna.get_power())+'dB',
                xlabel='Frequency (Hz)',
                ylabel='S21 (dBm)',
                path=full_path,
                close=close)
    ...
    def save(self):
        data_dict = {'S21':self.S21_to_dBm(), 'pna_power':self.pna.get_power(), 
                'f':self.f, 'pna_att':self.att}
        self.full_path = qf.save(self.properties, 'S21_measurement', data_dict,
