...
def single_scan(laser, daq_ai, daq_trig, wl_start=1550, wl_stop=1560, samples=100000, attenuation = 20, scan_speed=20.0,
                backwards=False, timeout=5.0):
    # private single scan function, returns scandata
    wavstart = wl_start
    wavstop = wl_stop
    if not ((laser.wav_min <= wavstart <= laser.wav_max) and (laser.wav_min <= wavstop <= laser.wav_max)):
        wl = None
        pd = None
        config = None
        print('input wavelength out of range. wavelength min: {}nm, wavelength max: {} nm'.format(laser.wav_min,
                                                                                                  laser.wav_max))
        return wl, pd, config
    ...
    if sample_rate > max_sample_rate:
        sweep_task.close()
        msg = 'The requested sample rate {} is faster than the max rate {}'.format(sample_rate, max_sample_rate)
        msg += '. Adjust scan settings to lower sample rate.'
        raise Exception(msg)
    sweep_task.timing.cfg_samp_clk_timing(float(sample_rate),
                                          samps_per_chan=samples)
    sweep_task.triggers.reference_trigger.cfg_dig_edge_ref_trig(
        trigger_source=daq_trig,
    ...
    wl = scandata['wav']
    pd = scandata['V']
    config = {'samples': samples, 'scan_speed': scan_speed, 'attenuation':attenuation, 'timeout': timeout}
    return wl, pd, config
