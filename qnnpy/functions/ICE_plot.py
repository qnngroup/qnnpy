...
def date_plot(path, date_start, date_end=datetime.date.today().isoformat(), plot=None):
    ...
    date_list = [datetime.datetime.strptime(folder_list[i], "%Y-%m-%d") \
     for i in range(len(folder_list)) \
     if datetime.datetime.strptime(folder_list[i], "%Y-%m-%d")>ds and \
     datetime.datetime.strptime(folder_list[i], "%Y-%m-%d") < de]
...
def save_cycle(x, y):
    ...
    for i in range(len(time_diff_index)-1):
        ...
        if temp_array[0,0] > 150 and temp_array[-1,0] > 150 and np.min(temp_array[:,0])<5: #first and last points are high temp
            trace_list.append(i)
    
    
    ''' Plot Sweeps '''
    for i in trace_list:
        time = np.asarray(x[time_diff_index[i]+1:time_diff_index[i+1]],dtype=np.datetime64)
        temp = np.asarray(y[time_diff_index[i]+1:time_diff_index[i+1]],dtype=np.float64)
