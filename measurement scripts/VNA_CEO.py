...
resonators.power_characterization(
    start_freq, stop_freq, num_points, powers, if_bandwidth
)
...
file_path, file_name = save_data_dict(
    data_dict,
    test_type=test_type,
    test_name=sample_name + "_" + device_name + "_sweep_other_current",
...
for v_out in V_test:
    ...
    SRS.set_output(False)
    # search the switching current in the ramping up
    Isw_full.append(I_device[np.argmax(np.array(V_device) > 0.05) - 1])
    V_device_full.append(V_device)
    I_device_full.append(I_device)
SRS2.set_output(False)
data_dict = {
    "V_device": V_device_full,
    "I_device": I_device_full,
    "V_source": V_source,
    "V_test": V_test,
    "R_srs": R_srs,
    "step": step,
...
for v in V_source:
    ...
    data_dict = {
        ...
        "f": f,
        "pna_att": pna_att,
        "I_other_source": V_source / R_srs,
        "I_other_device": iread,
        "V_other_device": vread,
    }
    file_path, file_name = save_data_dict(
        data_dict,
        test_type=test_type,
        test_name=sample_name + "_" + device_name + "_" + str(1e6 * v / R_srs) + "uA",
        filedir=filedirectry,
        zip_file=True,
    )
    plt.plot(f, 20 * np.log10(np.abs(S21)))
...
for pna_power in pna_powers:
    ...
    file_path, file_name = save_data_dict(
        data_dict,
        test_type=test_type,
        test_name=sample_name + "_" + device_name + "_" + str(pna_power) + "dBm",
        filedir=filedirectry,
        zip_file=True,
    )
...
bias_voltage = np.arange(0, 0.20, 0.02)
...
for i in range(len(bias_voltage)):
    plt.plot(
        f, 20 * np.log10(np.abs(S21[i])), label="DC bias={}V".format(bias_voltage[i])
    )
plt.legend()
...
data_dict = {"S11": S11, "S21": S21, "pna_power": pna_power, "f": f, "VNA_att": VNA_att}
file_path, file_name = save_data_dict(
    data_dict,
    test_type=test_type,
    test_name=sample_name + "_" + device_name,
