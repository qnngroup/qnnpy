...
SRS.set_output(False); sleep(.2)
SRS.set_output(True)
SRS.set_voltage(0)
sleep(.2)
SRS.set_voltage(v_bias)
sleep(1)
trigger_v, count_rate = counter.scan_trigger_voltage([0,.5], counting_time=.2, num_pts=50)
plt.semilogy(trigger_v, count_rate+1,'o')
plt.xlabel('Trigger level (V)'); plt.ylabel('Counts (cps)'); plt.title('Counts vs Bias\n' + test_name)
...
currents = np.arange(20e-6, 25e-6, .2e-6)
...
data_dict = {'DCR':DCR, 'currents':currents, 'counting_time':counting_time*iterations}
...
#currents = np.concatenate([currents1, currents2])
currents = np.arange(10e-6, 25e-6, .5e-6)
...
currents2 = np.arange(8e-6, 19e-6, .2e-6)
currents = np.concatenate([currents1, currents2])
...
data_dict = {'DCR':DCR, 'currents':currents, 'counting_time':counting_time, 'number_of_runs': number_of_runs}
file_path, file_name  = save_data_dict(data_dict, test_type = 'DCR', test_name = test_name+comments,
                        filedir = filedirectry)
plt.semilogy(1e6*currents, DCR, 'o', label='Dark count rate (DCR)')
plt.xlabel('Bias current (uA)'); plt.ylabel('Counts (Hz)'); plt.title('Counts vs Bias\n' + test_name)
