...
for y_step in range(1, y_steps+1):
    ...
    if y_step%2:
        ...
        for r in d.streamData():
            ...
            c2.extend(r['AIN1'])
            dataCount += 1
        vol=extract_useful_data(c1=c1,c2=c2, trigger=trigger, x_steps=x_steps, stream_frequency=stream_frequency, scan_frequency=scan_frequency)
        if y_step ==1:  
            vol_2d = np.array(vol)
        else:
        ...
        for r in d.streamData():
            ...
            dataCount += 1
        vol=extract_useful_data(c1=c1,c2=c2, trigger=trigger, x_steps=x_steps, stream_frequency=stream_frequency, scan_frequency=scan_frequency)
        vol_2d = np.vstack((vol_2d, np.array(vol[::-1])))
        #sleep(0.05+x_steps/scan_frequency)
